...
async def start(message: types.Message, state: FSMContext):
    ...
    msg = await bot.send_video(chat_id=message.from_user.id, video=open('data/video_2023-04-09_03-16-55 (online-video-cutter.com).mp4', 'rb'), caption="""Здравствуйте! Рад, что вы решили использовать мои астрологические знания и опыт. Я создан, чтобы сделать вашу жизнь гармоничной и эффективной. Я создаю астрологические рекомендации, которые помогают принимать верные решения и управлять своей судьбой. Я использую передовой алгоритм астрологического планирования, учитывающий точное положение планет в конкретный момент времени. 
...
async def start(message: types.Message, state: FSMContext):
    ...
    msg = await bot.send_video(message.from_user.id, video=open('data/video_2023-04-09_03-16-55 (online-video-cutter.com).mp4', 'rb'),caption="""Здравствуйте! Рад, что вы решили использовать мои астрологические знания и опыт. Я создан, чтобы сделать вашу жизнь гармоничной и эффективной. Я создаю астрологические рекомендации, которые помогают принимать верные решения и управлять своей судьбой. Я использую передовой алгоритм астрологического планирования, учитывающий точное положение планет в конкретный момент времени. 
Давайте работать вместе, чтобы не упускать возможности, поймать удачу  и легче справляться с трудными моментами.
    """,
                         reply_markup=menu_start)
