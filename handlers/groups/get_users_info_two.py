...
async def buy_subs_users(call: CallbackQuery, state: FSMContext):
    await call.message.edit_text(text="""Чтобы я мог дать Вам точные советы на основе астрологических данных, отправьте СВОЕ ТЕКУЩЕЕ местоположение. 

❗️Если задумаете переехать или улететь отдохнуть в другой часовой пояс в дальнейшем, то используйте кнопку ( 🚀Смена города)❗<b>Каждый раз при перелете отправляйте боту актуальную локацию.</b>
...
async def buy_subs_users(message: types.Message, state: FSMContext):
    try:
        ...
        return
    await message.answer("""Спасибо за предоставление необходимых данных!  

<b>Я успешно составил Вашу натальную карту, которая является уникальным астрологическим паспортом 🪐</b>

Теперь, чтобы получить персональный прогноз, выберите день или период, который Вас интересует 😊

Я с удовольствием <b>помогу Вам раскрыть все тайны Вашей судьбы и дам советы, как добиться успеха и счастья в жизни 💫</b>

Просто напишите мне, какой период вас интересует, и я подготовлю для вас подробный прогноз 📝
""",
