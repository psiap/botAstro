...
async def start(message: types.Message, state: FSMContext):
    await state.finish()
    await message.answer("""Чтобы рекомендации были корректными укажите ваше местоположение в тот день на который запрашиваете информацию. Для этого при помощи 📎, отправьте геопозицию, выбрав необходимое место на карте. Если точное место вам неизвестно, укажите ближайший населённый пункт.""")
