...
# Хранит количество дней в каждом месяце с 1901 по 2099. В местах с первого по тринадцатое хранится количество дней в каждом месяце (включая високосный месяц и 13 месяцев) .Значение 1 указывает, что месяц равен 30 дням.
# 0 в течение 29 дней. Цифры с 12-го по 15-й указывают месяц високосного месяца года, если он равен 0x0F, високосного месяца года не существует.
g_lunar_month_days = [
...
LUNAR_START_DATE, SOLAR_START_DATE = (1901, 1, 1), datetime(1901, 2,
                                                            19)  # Григорианская дата первого дня первого месяца 1901 года - 1901/2/19
LUNAR_END_DATE, SOLAR_END_DATE = (2099, 12, 30), datetime(2100, 2,
                                                          18)  # Григорианская дата 30 декабря 2099 года - 2100/2/8
...
# Рассчитать лунную дату на основе григорианского календаря и возврата (год, месяц, день, isLeap)
...
# Рассчитать григорианскую дату по лунному календарю и вернуть массив [datetime1, datetime2]. Если это високосный месяц, он может содержать две даты, в противном случае есть только одна.
def get_solar_date(year, month, day):
    ...
    if leap_month == month:
        return [SOLAR_START_DATE + timedelta(span_days),
                SOLAR_START_DATE + timedelta(span_days + lunar_month_days(year, leap_month))]
    else:
        return [SOLAR_START_DATE + timedelta(span_days)]